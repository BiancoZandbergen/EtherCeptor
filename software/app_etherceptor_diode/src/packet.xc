                 ...
                 )
{
  ...
  unsigned allow_forward=0, allow_arp=0, allow_icmp=0;
  led.blink(3);
  ...
  while (1)
  {
    ...
    case rx.packet_ready():
      ...
      if (allow_forward || (allow_arp && is_valid_arp_packet(rxbuf, packet_info.len)) ||
        (allow_icmp && is_valid_icmp_packet(rxbuf, packet_info.len)))
      {
    ...
    case stats.tx_total() -> unsigned stat:
      ...
      break;
    case config.update():
      allow_forward = config.forward();
      allow_arp = config.allow_arp();
