...
int main()
{
  ...
  interface info_log_if log_if[2];
  ...
  par {
    ...
    on tile[0]: packet_handler(eth1_i_cfg[CFG_TO_PACKET_HANDLER],
                            ...
                            led_if0, stats_if[0], log_if[0]);

    on tile[0].core[1]: led_handler(t0_p_led, led_if0);
    on tile[0]: serial_handler(i_tx, i_rx, stats_if, log_if);
